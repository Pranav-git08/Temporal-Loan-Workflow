...
from .config import DEFAULT_REMINDER_INTERVAL_DAYS
from .domain import build_kafka_message, evaluate_eligibility, price_loan
from .models import (
...
@activity.defn
async def persist_eligibility_result(
    application_id: str,
    ...
    def _run() -> None:
        repo.upsert_loan_record(
            application_id,
            {
                ...
                "status": "eligible" if eligibility_result.isEligible else "rejected",
                "updatedAt": datetime.utcnow().isoformat(),
            },
        )
...
async def persist_manual_review(application_id: str, review: ApprovalData) -> None:
    def _run() -> None:
        ...
        repo.upsert_loan_record(
            ...
            {
                "manualReview": review,
                "status": f"manual_{review.approvalStatus}",
                "updatedAt": datetime.utcnow().isoformat(),
            },
        )
...
async def process_loan_disbursement(
    application: LoanApplication, pricing: PricingResult
) -> DisbursementResult:
    ...
    def _run() -> None:
        ...
        repo.upsert_loan_record(
            ...
            {
                "disbursement": result,
                "status": "disbursed" if result.success else "disbursement_failed",
                "updatedAt": datetime.utcnow().isoformat(),
            },
        )
...
@activity.defn
async def publish_disbursement_message(
    application: LoanApplication,
    ...
    message = build_kafka_message(
        ...
        disbursement_status=disbursement.disbursementStatus,
...
async def send_periodic_email(loan_id: str, email_data: EmailNotification) -> None:
    def _run() -> None:
        ...
        repo.upsert_loan_record(
            loan_id,
            {"lastEmailNotification": email_data, "updatedAt": datetime.utcnow().isoformat()},
        )
...
async def cancel_remaining_notifications(loan_id: str) -> None:
    def _run() -> None:
        repo.upsert_loan_record(
            loan_id,
            {
                "emailNotificationsCancelled": True,
                "updatedAt": datetime.utcnow().isoformat(),
            },
        )
